...
const URGENCY_COLOR: Record<string, string> = {
  Critical: '#ff4466', High: '#ff8c42', Medium: '#ffd166', Low: '#06d6a0'
}
...
export default function Dashboard() {
  ...
  async function handleRemove(docId: string) {
    await removeDeadline(docId)
    setDeadlines(prev => prev.filter(d => (d as any)._docId !== docId))
  }
    ...
    <main className="min-h-screen py-10 px-4" style={{ background: 'var(--bg)' }}>
      <div className="max-w-3xl mx-auto flex flex-col gap-5">
        <div className="flex items-center justify-between">
          ...
          <a href="/" className="text-xs px-3 py-2 rounded-xl font-semibold"
            style={{ border: '1px solid var(--border)', color: 'var(--text2)' }}>← New deadline</a>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {sorted.map((d) => {
              ...
              const docId = (d as any)._docId ?? d.id
              return (
                <div key={docId} className="rounded-2xl p-4 flex flex-col gap-2"
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button onClick={() => handleRemove(docId)} className="text-xs" style={{ color: 'var(--text3)' }}>✕</button>
                  </div>
